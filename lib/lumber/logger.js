...
Logger.prototype.log = function() {
    ...
    args.cb = function() {
        ...
        if(done == self.transports.length) {
            if(errors.length && cb) {
                self.emit('logged', errors);
                cb(errors);
            } else if(cb) {
                self.emit('logged', null);
                cb(null);
            }
        }
    };
    ...
    self.transports.forEach(function(trans) {
        //if we aren't a silent level && this isn't a silent log && this log's level <= this transport's level
